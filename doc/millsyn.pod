...
=head2 Keywords
    ...
    ensure expose for func if invariant let proc require struct union use while
    case covariant given implement match protocol recursive switch this
    contravariant goto infix invariant is isnt postfix prefix return seq yield
    CHECK DEINIT INIT MAIN catch default defer do else fail last next redo try
...
=head3 Special Variables

There are certain built-in special variables. They cannot be assigned to unless
specified otherwise.
