...
preconditions which are checked upon construction.

    wrapper Hostname(String) {
        check this ~= [rx()|^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$]
    }

    let a = Hostname("example.com") # ok
    let c = Hostname("example%com") # precondition violation
    let b = Hostname("") # precondition violation
    outln(String(a)) # convert back to String
